...
public class MappingSerializer {
    ...
    public MappingSerializer(List<CreakModule<?>> cacheHandlers) {
    ...
    public void save(Path dir, RegistryFactory factory, List<CreakModule<?>> handlers, StepTask parent) {
        List<WeightedStageTask.WeightedStage> tasks = new ArrayList<>();
        for (CreakModule<?> value : handlers) {
        ...
        int i = 0;
        for (CreakModule<?> handler : handlers) {
    ...
    @SuppressWarnings({"unchecked", "rawtypes"})
    public boolean load(Path dir, RegistryReader reader, List<CreakModule<?>> handlers) {
        try {
            ByteBufferIO io = IOHelper.load(dir.resolve("mapping.bin"));
            for (CreakModule handler : handlers) {
                if (io.getByte() == 0 && handler.isActive()) {
                    Arthritis.LOG.info("Recaching as " + handler.getClass().getSimpleName() + " is now active.");
                    return false;
                } else {
                    Class<?> dataClass = handler.getDataClass();
